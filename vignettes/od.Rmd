...
The class `od` is a list of two elements, `U` for nodes and `E` for edges.
...
It is important that one column in `U`should exist that contains the key values, by which origins and destinations can be identified.
The key column is stored as an attribute named `od_id` in the `od` object.
...
Those are matched with the key column in $U$.
The origin and destination columns in `E`are stores as attributes `od_orig` and `od_dest` in the `od` object.
Optionally, `E` can be an `sf` object that contains the lines between origins and destinations.
This package contains a few functions to add lines automatically (with various options: straight lines, curved lines, half-lines).
When actual movement routes are available, those can obviously be used as well.

Note that users are add other columns to `U`and `E`. For aggregated OD data, `E`typically contains the flow counts, and grouping variables (e.g. the mode of transport).

In this package example is contained about Dutch commuting flows.
...
A more sophisticated map using similar data is shown below:

```{r,echo=FALSE, out.width = "100%"}
knitr::include_graphics("images/commuting_ODDV_map3_v2.jpg")
```

See an [interactive prototype](http://www.mtennekes.nl/viz/commutingNL/) of this visualization.
